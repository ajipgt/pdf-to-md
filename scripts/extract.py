...
RE_DEFINISI = re.compile(r'^"?([A-Z][^"]+?)"?\s+adalah\s+', re.IGNORECASE)
...
class Ayat:
    nomor: str
    teks: str
    huruf: list[tuple[str, str]] = field(default_factory=list)
...
# ── Ekstrak wikilinks dari Pasal 1 (definisi) ─────────────────────────────────
def extract_wikilinks(pasal: Pasal) -> list[str]:
    """Dari pasal definisi, ekstrak konsep sebagai wikilinks Obsidian."""
    links: set[str] = set()
    semua_teks: list[str] = []

    for ayat in pasal.ayat:
        semua_teks.append(ayat.teks)
        for _, ht in ayat.huruf:
            semua_teks.append(ht)
    semua_teks.extend(pasal.teks_langsung)

    for teks in semua_teks:
        m = RE_DEFINISI.match(teks)
        if m:
            konsep = m.group(1).strip().strip('"')
            if 3 < len(konsep) < 60:
                links.add(konsep)
...
def find_wikilinks_in_text(teks: str, semua_konsep: list[str]) -> str:
    """Ganti penyebutan konsep dalam teks dengan [[konsep]] Obsidian."""
    for konsep in semua_konsep:
        # Word boundary, case-sensitive untuk istilah hukum
        pattern = re.compile(rf'\b({re.escape(konsep)})\b')
        teks = pattern.sub(r'[[\1]]', teks, count=1)
    return teks


# ── Generate Markdown per pasal ───────────────────────────────────────────────
def render_bunyi_pasal(pasal: Pasal, semua_konsep: list[str]) -> str:
    """Render isi pasal ke format blockquote MD."""
    parts: list[str] = []

    if pasal.teks_langsung:
        gabung = " ".join(pasal.teks_langsung)
        parts.append(f"> {find_wikilinks_in_text(gabung, semua_konsep)}")

    for ayat in pasal.ayat:
        teks = find_wikilinks_in_text(ayat.teks, semua_konsep)
        parts.append(f"> **({ayat.nomor})** {teks}")
        for huruf, ht in ayat.huruf:
            ht_link = find_wikilinks_in_text(ht, semua_konsep)
            parts.append(f">   **{huruf}.** {ht_link}")

    return "\n".join(parts) if parts else "> *(isi pasal tidak terdeteksi)*"
...
def generate_pasal_md(pasal: Pasal, reg: Regulasi, semua_konsep: list[str]) -> str:
    ...
    # Wikilinks: konsep yang muncul di pasal ini
    teks_gabung = " ".join(
        [a.teks for a in pasal.ayat]
        + [ht for a in pasal.ayat for _, ht in a.huruf]
        + pasal.teks_langsung
    )
    konsep_muncul = [k for k in semua_konsep if k.lower() in teks_gabung.lower()]
...
---"""
    ...
    if konsep_muncul:
        konsep_section = "## Konsep Terkait\n\n" + "\n".join(
            f"- [[{k}]]" for k in konsep_muncul
        )
    else:
        konsep_section = "## Konsep Terkait\n\n*(tidak ada)*"
    ...
    return "\n\n".join([frontmatter, heading, bunyi, konsep_section, catatan]) + catatan_nav
...
def main() -> None:
    ...
    # Ekstrak semua konsep dari Pasal 1 (definisi)
    pasal_1 = next((p for p in reg.pasal_list if p.nomor == "1"), None)
    semua_konsep = extract_wikilinks(pasal_1) if pasal_1 else []
    print(f"  {len(semua_konsep)} konsep/definisi: {semua_konsep[:5]}...")
    ...
    for pasal in reg.pasal_list:
        slug = f"pasal-{pasal.nomor.lower()}"
        md_content = generate_pasal_md(pasal, reg, semua_konsep)
