...
# Deteksi huruf inline di teks ayat: ' a. teks b. teks'
...
# ── Deteksi huruf inline ──────────────────────────────────────────────────────
def split_inline_huruf(teks: str) -> tuple[str, list[Huruf]]:
    """
    Pisahkan huruf a. b. c. yang nyempil di teks ayat.

    Input:  'dilakukan dengan ketentuan: a. paling lama 13 hari b. paling lama 13 hari'
    Output: ('dilakukan dengan ketentuan:', [Huruf('a', 'paling lama 13 hari'), ...])
    """
...
def flush_pasal(current_pasal: Pasal | None, current_ayat: Ayat | None, reg: Regulasi) -> None:
    if current_pasal is None:
        return
    if current_ayat:
        # Scan teks ayat untuk huruf inline yang nyempil
...
def parse_regulasi(lines: list[str], reg: Regulasi) -> None:
    ...
    while i < len(lines):
        ...
        if m_ayat:
            if current_ayat:
                # Scan huruf inline sebelum simpan ayat sebelumnya
                if not current_ayat.huruf:
                    teks_bersih, huruf_inline = split_inline_huruf(current_ayat.teks)
                    if huruf_inline:
                        current_ayat.teks  = teks_bersih
